...
def delete_all_volumes():
    # Create a session and an EC2 client (waiters are available through the client interface)
    ec2_client = boto3.client('ec2')
    ...
    for volume in volumes:
        # Only delete volumes that are not 'in-use'
        if volume.state != 'available':
            print(f"Waiting for volume {volume.id} to become available (current state: {volume.state})")
            # Use the 'wait_until_available' waiter
            ec2_client.get_waiter('volume_available').wait(VolumeIds=[volume.id])

        # Once the volume is available, delete it
